...
export function Logo({ size = 'md', variant = 'onLight', withText = true }: LogoProps) {
  const onDark = variant === 'onDark';
  const box =
    size === 'lg' ? 'h-16 w-16 rounded-2xl' : size === 'sm' ? 'h-8 w-8 rounded-lg' : 'h-10 w-10 rounded-xl';
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      {withText && <span className={`font-bold tracking-tight ${textSize} ${textColor}`}>CampusFlow</span>}
    </div>
  );
}
